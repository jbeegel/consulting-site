// Live Opportunity Audit — overlays real DataForSEO data onto the demo
// baseline, then re-runs the full assembly so every number, score, and
// sentence in the audit is re-derived from the real data. Each provider
// degrades independently, demo fills the gaps, and the meta block says
// exactly what's real.
...
const MAX_SERP_SCANS = 6; // one per cluster — the cost ceiling per live audit
...
export async function buildAuditMaybeLive(
  ...
  const [ranked, aiVols, difficulty, competitors, overview] = await Promise.all([
  ...
  // --- real ranking positions + volumes where the audited set overlaps ---
  if (ranked) {
  ...
  // --- AI-assistant search volumes (the AI keyword dataset) ---
  if (aiVols) {
  ...
  const sources = [
    ...
    "Simulated where noted: demand for keywords DataForSEO hasn't observed for this domain, and all revenue modeling assumptions",
  ];
  if (overview) {
    sources.push(
